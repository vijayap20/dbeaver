...
public class ClaudeTranslateHandler extends AbstractHandler {
	
	private static final String aiProvider = "anthropic";

    @Override
    public Object execute(ExecutionEvent event) throws ExecutionException {
        AIFeatures.SQL_AI_POPUP.use();

        if (DBWorkbench.getPlatform().getPreferenceStore().getBoolean(AICompletionConstants.AI_CLAUDE_DISABLED)) {
            return null;
        }
...
//        DAICompletionEngine engine;
        ClaudeCompletionEngine engine;
        try {
            engine = AIEngineRegistry.getInstance().getClaudeCompletionEngine("anthropic");
        } catch (Exception e) {
        ...
        DAICompletionSettings settings = new DAICompletionSettings(dataSourceContainer, aiProvider);
